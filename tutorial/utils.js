function displayInteractive(holder, focusModule, showOnly, fullBase, inpDesc){
    ...
    let interactive = studio.newStudio(holder, [window.innerWidth*0.8,window.innerHeight*0.8,], state)
    d3.select(`${holder} #module_1`).node().parentElement.remove()
    interactive.__data__.moduleHolders.module_1.remove()
    delete interactive.__data__.moduleHolders.module_1
    d3.select(`${holder} #${focusModule}`).node().click()
    const svgData = interactive.__data__.moduleHolders[focusModule].querySelector('svg').__data__
